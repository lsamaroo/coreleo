...
public final class StringUtil {
	...
	public static final boolean isPalindrome( Object obj ){
		...
		char[] c = String.valueOf(obj).toCharArray();
		int start = 0;
		int end = c.length-1;
	...
	/**
	 * @return - true if it contains at least one whitespace character as
	 *         defined by regex--space, tab, etc.
	 */
	...
	/**
	 * 
	 * Check to string to see if it contains any of the characters specified in
	 * the char array.
	 * 
	 * @param string
	 *            - the String to check.
	...
	public static final int indexOfAnyBut(String str, String allowedCharacters) {
		if (str == null || allowedCharacters == null || isEmpty(str)
				|| isEmpty(allowedCharacters)) {
			return -1;
		}
            ...
            while (wcsIdx < wcs.length) {
                ...
                if (wcs[wcsIdx].equals("?")) {
                    ...
                    if (anyChars) {
                        ...
                        int repeat = text.indexOf(wcs[wcsIdx], textIdx + 1);
                        if (repeat >= 0) {
