...
public final class NumberUtil {
	...
	public static String toBinaryString( int x, int bits ){
		...
		while( q != 0 ){
			...
			int temp = isNegative ? Math.abs(r - 1) : r;
			builder.insert(0, temp);
		}
		
		char prefixChar = isNegative ? '1' : '0';
		int numTimes = bits - builder.length();
    ...
    public static final boolean inRange(Number x, Number startRange, Number endRange) {
		...
		int compareMin = ((Comparable) startRange).compareTo(x);
		int compareMax = ((Comparable) endRange).compareTo(x);
    ...
    // perform formatting, accuracy is lost when dealing with large #s
