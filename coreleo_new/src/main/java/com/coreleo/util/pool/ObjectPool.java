 ...
 * 
 */
public abstract class ObjectPool extends AbstractPool {
	private final ExecutorService threadPool = Executors.newSingleThreadExecutor();
	
	private final Map<Object, MetaData> locked;
	private final Map<Object, MetaData> unlocked;
	private final int minObjectsInPool;
	private final int maxObjectsInPool;
	/** Specifies the interval in milliseconds after which an idle connection is discarded. A value of zero means allow to idle indefinitely.*/
	...
	protected ObjectPool(String name, long objectIdleTimeOut, long objectLife, long reapTime, int minObjectsInPool, int maxObjectsInPool ) {
		super(name);
		this.locked = new Hashtable<Object, MetaData>();
		this.unlocked = new Hashtable<Object, MetaData>();
	...
	public synchronized Object checkOut() throws Exception {
		...
		Object object;
		MetaData metaData;
		Map.Entry<Object, MetaData> entry;
		
		if (unlocked.size() > 0) {
			try{
				for (Iterator<Map.Entry<Object, MetaData>> i = unlocked.entrySet().iterator(); i.hasNext(); ) {  
				    entry = i.next();
				    object = entry.getKey();
					if (validate(object)) {
	...
	public synchronized void checkIn(Object obj) {
		...
		if (obj != null ) {
			...
			int totalObjectsInPool = locked.size() + unlocked.size();
		...
		this.numberOfObjectsLocked = locked.size();
		this.numberOfObjectsUnLocked = unlocked.size();
		LogUtil.trace( this, "ObjectPool:checkIn - name=" + super.getName() + " numberOfObjectsLocked=" + numberOfObjectsLocked + " numberOfObjectsUnLocked= " + numberOfObjectsUnLocked + " totalObjectsInPool=" + (numberOfObjectsLocked+numberOfObjectsUnLocked) );
	}
	...
	protected synchronized void cleanUp() {
		...
		try{
		  for (Iterator<Map.Entry<Object, MetaData>> i = unlocked.entrySet().iterator(); i.hasNext(); ) {  
				...
				object = entry.getKey();
				metaData = entry.getValue();
				long timeIdleInPool = (now - (metaData != null ? metaData.getAccessed() : 0));
				long timePassedSinceCreated = (now - (metaData != null ? metaData.getCreated() : 0));
				boolean hasIdled = (objectIdleTimeOut != 0 && timeIdleInPool > objectIdleTimeOut);
				boolean hasGrownOld = (objectLife != 0 && timePassedSinceCreated > objectLife);
				if ( hasIdled || hasGrownOld ) {
					LogUtil.debug(this, "ObjectPool:cleanUp - name=" + super.getName() + " expiring object=" + object + " metaData=" + metaData + " timeIdleInPool=" + timeIdleInPool+ " hasIdled=" + hasIdled + " timePassedSinceCreated=" + timePassedSinceCreated + " hasGrownOld=" + hasGrownOld );
					i.remove();
					expire(object);
				}
			}
