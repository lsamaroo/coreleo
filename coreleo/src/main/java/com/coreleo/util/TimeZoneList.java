...
public class TimeZoneList {
	...
	private static final HashMap<String, String> WINDOWSSTANDARDTIMEZONENAMES_TO_JAVA_TIMEZONE_IDS = new HashMap<String, String>();
	...
	static {
		...
		ZONEMAPPINGS.add(new TimeZoneMapping("Dateline Standard Time", "Etc/UTC+12", "(UTC -12:00) International Date Line West"));
		...
		ZONEMAPPINGS.add(new TimeZoneMapping("UTC Standard Time", "Europe/London", "(UTC) Dublin, Edinburgh, Lisbon, London"));
	...
	private TimeZoneList() {
		...
		for (TimeZoneMapping zoneMapping : ZONEMAPPINGS) {
			...
			if (availableIdsSet.contains(id)) {
				...
				timeZones.add(new TimeZoneWithDisplayNames(timeZone, zoneMapping.getWindowsDisplayName(), zoneMapping.getWindowsStandardName()));
	...
	public String getJavaTimeZoneId( String windowStandardName ){
